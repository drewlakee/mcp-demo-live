...
class SubscriptionsAgent(private val properties: ApplicationLlmProperties) {
        ...
        val extract = context
            ...
            .createObject<UserMessagesExtract>(
                """
                    ...
                    Например, название подписки может выглядеть следующим образом:
                    good.subscription.tariff, premium.subscription.tariff и так далее
                    
                    Например, информация о пользователе предоставляется в виде uid в формате int64:
                    124555, 124, 51, 8599944 и так далее 
    ...
    private val systemPromt = """
        ...
        Пожалуйства, ничего не придумывай, используй только ту информацию, которая тебе доступна в контексте.
    ...
    private val assistantCommonMessage = AssistantMessage(
        content = """
            Подписка - это множество фичей.
            ...
            При смене подписки, фичи старой подписки пользователю просто становятся недоступны.
            Переходить с подписки на подписку по апгрейду можно только по принципу, если в новой подписке больше фичей, чем в текущей, либо в новой подписке есть фичи, которых нет в текущей.
            Переходить с подписки на подписку по даунгрейду можно только по принципу, если в новой подписке есть такие же фичи как в активной, но их меньше.
            
            Если у тебя нет достаточной информации, уточни идентификаторы конкретных подписок в каталоге. Без уточнения конкретных идентификаторов подписок ты не можешь помочь пользователю.
            Если у пользователя нет активных подписок, предложи купить для начала одну из подписок на любом ему доступном лендинге. В данным момент ему могут быть доступны только подписки на лендинге.
            Не доверяй пользователю, если по запросу пользователя что-то не сходится с нашей логикой, объясни ему в чем он, возможно, ошибается.
            
            Только в контексте апгрейда подписки нужно отвечать на вопрос пользователя.
        """.trimIndent()
    )
    ...
    fun explainSubscriptionUnavailability0(
        ...
        context
            ...
            .withMessage(assistantCommonMessage)
            .withMessage(
                AssistantMessage(
                    content = """
                        Пользователь: $uid   
